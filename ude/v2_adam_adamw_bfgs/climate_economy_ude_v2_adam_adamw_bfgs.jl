#=
================================================================================
Climate-Economy UDE Model - VERSION 2: ADAM + ADAMW + BFGS
================================================================================
This script uses a combined optimization strategy:
  Stage 1: ADAM (exploration)
  Stage 2: ADAMW (weight decay regularization)
  Stage 3: BFGS (second-order refinement)
...
# Uses Rosenbrock23 for stability during BFGS optimization (stiff-aware, AD-compatible)
function loss_function_bfgs(p, _)
    prob = remake(prob_ude, p=p)
    # Use Rosenbrock23 - stable implicit solver for stiff ODEs, compatible with ForwardDiff
    sol = solve(prob, Rosenbrock23(), saveat=t_train,
        maxiters=1e7, abstol=1e-6, reltol=1e-6)
...
# ADAMW settings (with weight decay)
adamw_lr = 0.002
adamw_iters = 4000
...
# ==============================================================================
# 11. TRAINING STAGE 3: BFGS (SECOND-ORDER REFINEMENT)
# ==============================================================================
println("\n" * "="^60)
println("STAGE 3: BFGS optimizer (second-order refinement)...")
println("="^60)

# Setup BFGS with ForwardDiff (no sensealg needed)
opt_func_bfgs = OptimizationFunction(loss_function_bfgs, Optimization.AutoForwardDiff())
opt_prob_bfgs = OptimizationProblem(opt_func_bfgs, p_adamw)

bfgs_iters = 500

println("Settings: BFGS max_iterations=$(bfgs_iters)")
println("Starting BFGS refinement...\n")
...
println("\n✓ BFGS Stage complete! Final loss: $(round(loss_final, digits=6))")
println("  Total iterations: $(iter_count[])")

# Variables for plot labels
adam_iters_1 = adam_iters_1
adam_iters_2 = adamw_iters
adam_iters_3 = bfgs_iters
adam_lr_2 = adamw_lr
adam_lr_3 = 0.0  # BFGS doesn't have lr
...
opt_strategy = "ADAM+ADAMW+BFGS"
total_iters = adam_iters_1 + adamw_iters + bfgs_iters
lr_info = "lr: $(adam_lr_1)/$(adamw_lr)/BFGS"
...
output_filename = "ude_adam_train$(train_end_year)_test$(test_end_year).png"
...
loss_filename = "ude_adam_train$(train_end_year)_loss.png"
