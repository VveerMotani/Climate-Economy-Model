#=
================================================================================
Climate-Economy UDE Model - VERSION 1: 3-STAGE ADAM (BASELINE)
================================================================================
This script uses a 3-stage ADAM optimization strategy:
  Stage 1: Exploration (lr=0.005, 8000 iters)
  Stage 2: Convergence (lr=0.001, 5000 iters)
  Stage 3: Refinement (lr=0.0002, 3000 iters)

Train/Test Split: 1959-2015 (train) | 2011-2023 (test)
================================================================================
=#
...
# ==============================================================================
# TRAIN-TEST SPLIT: Train on 1959-2015, Test/Predict on 2011-2023
# ==============================================================================
train_end_year = 2019
test_end_year = 2023
...
full_idx = findall(y -> y <= test_end_year, years_all)  # 1959-2023

# Training data (1959-2015)
...
# Time spans
tspan_train = (t_train[1], t_train[end])  # Training: 1959-2015
...
# Y grows from 1 to ~8.4 over 51 years (1959-2015)
...
# Create ODE problem for TRAINING (1959-2015)
...
function loss_function(p, _)
    # Solve ODE with current parameters (TRAINING PERIOD ONLY: 1959-2015)
...
# ==============================================================================
# 9. TRAINING STAGE 1: ADAM OPTIMIZER (EXPLORATION PHASE)
# ==============================================================================
println("\n" * "="^60)
println("STAGE 1: Training with ADAM optimizer (exploration)...")
println("="^60)
...
# ADAM optimization settings - Stage 1 (Higher lr for exploration)
adam_lr_1 = 0.005   # Increased from 0.0005 for better exploration
adam_iters_1 = 8000  # Increased from 4000

println("Settings: lr=$(adam_lr_1), max_iterations=$(adam_iters_1)")
println("Starting ADAM training (Stage 1 - Exploration)...\n")

# Reset iteration counter
iter_count[] = 0

# Run ADAM optimization - Stage 1
result_adam_1 = solve(opt_prob, OptimizationOptimisers.Adam(adam_lr_1),
    maxiters=adam_iters_1,
    callback=callback)

p_adam_1 = result_adam_1.u
loss_adam_1 = result_adam_1.objective

println("\n✓ ADAM Stage 1 complete!")
println("  Loss after Stage 1: $(round(loss_adam_1, digits=6))")
println("  Iterations: $(iter_count[])")

# ==============================================================================
# 10. TRAINING STAGE 2: ADAM FINE-TUNING (CONVERGENCE PHASE)
# ==============================================================================
println("\n" * "="^60)
println("STAGE 2: Fine-tuning with ADAM (convergence)...")
println("="^60)

# Create new optimization problem with Stage 1 results
opt_prob_2 = OptimizationProblem(opt_func, p_adam_1)

# ADAM optimization settings - Stage 2 (Medium lr for convergence)
adam_lr_2 = 0.001   # Increased from 0.0002
adam_iters_2 = 5000  # Increased from 2000

println("Settings: lr=$(adam_lr_2), max_iterations=$(adam_iters_2)")
println("Starting ADAM fine-tuning (Stage 2 - Convergence)...\n")

# Run ADAM optimization - Stage 2
result_adam_2 = solve(opt_prob_2, OptimizationOptimisers.Adam(adam_lr_2),
    maxiters=adam_iters_2,
    callback=callback)

p_adam_2 = result_adam_2.u
loss_adam_2 = result_adam_2.objective

println("\n✓ ADAM Stage 2 complete!")
println("  Loss after Stage 2: $(round(loss_adam_2, digits=6))")

# ==============================================================================
# 11. TRAINING STAGE 3: ADAM FINAL POLISH (REFINEMENT PHASE)
# ==============================================================================
println("\n" * "="^60)
println("STAGE 3: Final polish with ADAM (refinement)...")
println("="^60)

# Create new optimization problem with Stage 2 results
opt_prob_3 = OptimizationProblem(opt_func, p_adam_2)

# ADAM optimization settings - Stage 3 (Low lr for final refinement)
adam_lr_3 = 0.0002   # Slightly increased from 0.0001
adam_iters_3 = 3000   # Increased from 1500

println("Settings: lr=$(adam_lr_3), max_iterations=$(adam_iters_3)")
println("Starting ADAM final polish (Stage 3 - Refinement)...\n")

# Run ADAM optimization - Stage 3
result_adam_3 = solve(opt_prob_3, OptimizationOptimisers.Adam(adam_lr_3),
    maxiters=adam_iters_3,
    callback=callback)

p_final = result_adam_3.u
loss_final = result_adam_3.objective
...
opt_strategy = "3-Stage ADAM"
total_iters = adam_iters_1 + adam_iters_2 + adam_iters_3
lr_info = "lr: $(adam_lr_1)→$(adam_lr_2)→$(adam_lr_3)"
...
output_filename = "ude_3_stage_adam_train$(train_end_year)_test$(test_end_year).png"
...
loss_filename = "ude_3_stage_adam_train$(train_end_year)_loss.png"
...
savefig(p3, joinpath(@__DIR__, "plot_hidden_dynamics.png"))
println("✓ Saved: plot_hidden_dynamics.png")
...
println("  3. plot_hidden_dynamics.png        - NN(C) vs theoretical log(C)")
