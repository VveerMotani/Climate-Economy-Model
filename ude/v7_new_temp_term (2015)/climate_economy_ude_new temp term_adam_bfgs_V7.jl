#=
================================================================================
Climate-Economy UDE Model - VERSION 6: ADAM + BFGS (Rosenbrock23)
================================================================================
Simple 2-stage optimization:
  Stage 1: ADAM (exploration, 10000 iters)
  Stage 2: BFGS (second-order refinement, 200 iters)

Solver: Rosenbrock23() throughout (stiff-aware, AD-compatible)
Train/Test Split: 1959-2015 (train) | 2016-2023 (test)
================================================================================
=#

# Version identifier for output
const VERSION = "V7"
...
# ==============================================================================
# TRAIN-TEST SPLIT: Train on 1959-2015, Test/Predict on 2016-2023
# ==============================================================================
train_end_year = 2015
test_end_year = 2023
...
try
    sol_test = solve(prob_ude, Rosenbrock23(), saveat=t_train)
...
function loss_function(p, _)
    # Solve ODE with current parameters using Rosenbrock23
    prob = remake(prob_ude, p=p)
    sol = solve(prob, Rosenbrock23(), saveat=t_train,
...
# Loss function for BFGS (uses ForwardDiff, Rosenbrock23)
function loss_function_bfgs(p, _)
    prob = remake(prob_ude, p=p)
    sol = solve(prob, Rosenbrock23(), saveat=t_train, maxiters=1e7)
...
# ==============================================================================
# 9. TRAINING STAGE 1: ADAM OPTIMIZER
# ==============================================================================
println("\n" * "="^60)
...
println("\n✓ ADAM complete! Loss: $(round(loss_adam, digits=6))")

# ==============================================================================
# 10. TRAINING STAGE 2: BFGS REFINEMENT
# ==============================================================================
println("\n" * "="^60)
println("STAGE 2: BFGS optimizer (second-order refinement)...")
println("="^60)

# Setup BFGS with ForwardDiff
opt_func_bfgs = OptimizationFunction(loss_function_bfgs, Optimization.AutoForwardDiff())
opt_prob_bfgs = OptimizationProblem(opt_func_bfgs, p_adam)

bfgs_iters = 200

println("Settings: BFGS max_iterations=$(bfgs_iters)")
println("Starting BFGS refinement (this may take a while)...\n")

result_bfgs = solve(opt_prob_bfgs, OptimizationOptimJL.BFGS(),
    maxiters=bfgs_iters,
    callback=callback)

p_final = result_bfgs.u
loss_final = result_bfgs.objective

println("\n✓ BFGS complete! Final loss: $(round(loss_final, digits=6))")
println("  Total iterations: $(iter_count[])")

# Variables for plot labels
adam_iters_2 = bfgs_iters
adam_iters_3 = 0
adam_lr_2 = 0.0
adam_lr_3 = 0.0

# ==============================================================================
# 11. FINAL SOLUTION AND PREDICTION
# ==============================================================================
println("\n" * "="^60)
println("Computing final solution and predictions...")
println("="^60)

# Solve ODE with optimized parameters using Rosenbrock23
prob_final = remake(prob_ude_full, p=p_final)
sol_final = solve(prob_final, Rosenbrock23(), saveat=t_data)
...
opt_strategy = "ADAM+BFGS"
total_iters = adam_iters_1 + bfgs_iters
lr_info = "ADAM lr=$(adam_lr_1), BFGS iters=$(bfgs_iters)"
split_info = "Train: 1959-$(train_end_year) | Test: $(train_end_year+1)-$(test_end_year)"

# Create main title with VERSION
main_title = "Climate-Economy UDE Model [$(VERSION)]\\n" *
             "Strategy: $(opt_strategy) | Total Iters: $(total_iters) | Solver: Rosenbrock23\\n" *
...
# Save with descriptive filename including VERSION
output_filename = "ude_$(VERSION)_train$(train_end_year)_test$(test_end_year).png"
...
p2 = plot(1:length(loss_history), loss_history,
    xlabel="Iteration",
    ylabel="MSE Loss",
    title="Training Loss [$(VERSION)] | $(opt_strategy) | $(total_iters) iters",
    linewidth=2,
    color=:green,
    legend=false,
    yscale=:log10,
...
vline!(p2, [adam_iters_1], linestyle=:dash, color=:orange, linewidth=1.5, label="ADAM→BFGS")

loss_filename = "ude_$(VERSION)_train$(train_end_year)_loss.png"
