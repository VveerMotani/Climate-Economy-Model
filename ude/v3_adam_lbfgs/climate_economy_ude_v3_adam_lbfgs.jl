#=
================================================================================
Climate-Economy UDE Model - VERSION 3: ADAM + L-BFGS
================================================================================
This script uses a two-phase optimization strategy:
  Stage 1: ADAM (exploration with momentum)
  Stage 2: L-BFGS (quasi-Newton second-order optimizer)
...
# Loss function for L-BFGS (uses ForwardDiff, no sensealg needed) - TRAINING DATA ONLY
# Uses stiff-aware solver for stability during L-BFGS optimization
function loss_function_bfgs(p, _)
    prob = remake(prob_ude, p=p)
    # Use AutoVern7 with Rodas5 fallback for stiff regions
    sol = solve(prob, AutoVern7(Rodas5()), saveat=t_train,
...
# ADAM settings - longer exploration
adam_lr_1 = 0.01  # Higher lr for faster initial exploration
adam_iters_1 = 10000
...
# ==============================================================================
# 10. TRAINING STAGE 2: L-BFGS (QUASI-NEWTON REFINEMENT)
# ==============================================================================
println("\n" * "="^60)
println("STAGE 2: L-BFGS optimizer (quasi-Newton refinement)...")
println("="^60)

# Setup L-BFGS with ForwardDiff
opt_func_lbfgs = OptimizationFunction(loss_function_bfgs, Optimization.AutoForwardDiff())
opt_prob_lbfgs = OptimizationProblem(opt_func_lbfgs, p_adam)

lbfgs_iters = 1000

println("Settings: L-BFGS max_iterations=$(lbfgs_iters)")
...
p_final = result_lbfgs.u
loss_final = result_lbfgs.objective

println("\n✓ L-BFGS Stage complete! Final loss: $(round(loss_final, digits=6))")
println("  Total iterations: $(iter_count[])")

# Variables for plot labels
adam_iters_1 = adam_iters_1
adam_iters_2 = lbfgs_iters
adam_iters_3 = 0
adam_lr_2 = 0.0  # L-BFGS adaptive
adam_lr_3 = 0.0
...
opt_strategy = "ADAM+L-BFGS"
total_iters = adam_iters_1 + lbfgs_iters
lr_info = "lr: $(adam_lr_1)/L-BFGS"
